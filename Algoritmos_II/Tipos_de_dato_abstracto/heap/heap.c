...
void upheap(void *arreglo[], size_t i_actual, cmp_func_t cmp){

	if(i_actual == 0) return;

	size_t i_padre = (i_actual - 1) / 2;

	int cmp_result = cmp(arreglo[i_actual], arreglo[i_padre]);

	if(cmp_result <= 0) return;

	swap_heap(arreglo, i_actual, i_padre);
	upheap(arreglo, i_padre, cmp);
}
...
void downheap(void *arreglo[], size_t i_actual, size_t cant, cmp_func_t cmp){
	...
	int cmp_result = cmp(arreglo[hijo_max], arreglo[i_actual]);

	if(cmp_result <= 0) return;

	swap_heap(arreglo, hijo_max, i_actual);
	downheap(arreglo, hijo_max, cant, cmp);
}
...
void heapify(void *arreglo[], size_t i_actual, size_t cant, cmp_func_t cmp){
   
    downheap(arreglo, i_actual, cant, cmp); //i_actual arranca en el ultimo     

	if(i_actual <= 0) return;

    heapify(arreglo, i_actual-1, cant, cmp);
...
void wrapper_heap_sort(void *elementos[], size_t i_actual, size_t cant, cmp_func_t cmp){
    ...
    wrapper_heap_sort(elementos, cant-1, cant, cmp);
}

void heap_sort(void *elementos[], size_t cant, cmp_func_t cmp){

	heapify(elementos, cant-1, cant, cmp);
