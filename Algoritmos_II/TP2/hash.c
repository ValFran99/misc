...
void *saltar_vacias(hash_iter_t *iter){
...
bool hash_iter_avanzar(hash_iter_t *iter){ 
	...
	if(!lista_esta_vacia(iter->hash->tabla[iter->index])){
		lista_iter_avanzar(iter->lista_iter);
		iter->actual = lista_iter_ver_actual(iter->lista_iter);
	}	


	if(hash_iter_al_final(iter)){ //En caso de que este en mi anteultimo, avanzo a NULL. Avanzar fue True, pero ya no busco listas mas adelante
        return true;
    }
...
bool hash_iter_al_final(const hash_iter_t *iter){
	...
	if(iter->index+1 >= iter->hash->capacidad){ //Si mi iterador esta al final de la lista, y mi hashed(siendo la lista en la que estoy) +1 es igual a la capacidad, estoy en la ultima lista de mi hash
		return true;
	}
